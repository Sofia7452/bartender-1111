...
export const getColor = (color: string, shade: number = 500) => {
  ...
  const actualShade = colorShade ? parseInt(colorShade) : shade;
  return theme.colors[colorName as keyof typeof theme.colors]?.[actualShade as keyof typeof theme.colors.primary] || color;
};
